...
var compare = function (a, b) {
  if (
    ...
    var a1 =
      a.split(".").reduce(function (ipInt, octet) {
        return (ipInt << 8) + parseInt(octet, 10);
      }, 0) >>> 0;
    if (b === null) var b1 = 0;
    else
      var b1 =
        b.split(".").reduce(function (ipInt, octet) {
          return (ipInt << 8) + parseInt(octet, 10);
        }, 0) >>> 0;
    return a1 - b1;
  } else if (isNaN(a)) return a.toString().localeCompare(b);
  else if (isNaN(b)) return -1 * b.toString().localeCompare(a);
...
function compareVersion(vers1, vers2) {
  ...
  if (typeof vers1 !== "string") return false;
  if (typeof vers2 !== "string") return false;
  vers1 = vers1.split(".");
  vers2 = vers2.split(".");
...
class CellFormatters {
  ...
  fw(data) {
    if (data != null) return data.replace("@", "&shy;@");
  }
  version(data) {
    if (data != null) return this.fw(data.replace("#", "<br>"));
  }
  signalIcon(data) {
    let icon, color;
    if (data >= -50) {
    ...
    if (this.cfg.sort_by) {
      ...
      let sort_conf = sort_cols.map((sort_col) => {
        ...
        out.idx = this.cols.findIndex((col) =>
          ["id", "attr", "prop", "attr_as_list", "data"].some(
            (attr) => attr in col && out.col == col[attr]
          )
        );
        return out;
      });
      ...
      // sort conf checks
      sort_conf = sort_conf.filter(
        (conf) => conf.idx !== -1 && conf.idx !== null
      );
...
function show_deprecation_message() {
  if (!show_deprecation) return;
  // console.log(), console.warn(), console.error(), alert()
  console.log(
    "DEPRECATION WARNING: 'multi', 'attr', 'attr_as_list', 'prop' as column " +
...
class DataRow {
  constructor(entity, strict, raw_data = null, hass) {
    this.entity = entity;
    this.hidden = false;
    this.strict = strict;
  ...
  get_raw_data(col_cfgs) {
    this.raw_data = col_cfgs.map((col) => {
      ...
      if ("data" in col) {
        ...
        for (let item of col.multi) col_getter.push([item[0], item[1]]);
      } else if ("attr" in col || "prop" in col || "attr_as_list" in col) {
        show_deprecation_message();

        if ("attr" in col) col_getter.push(["attr", col.attr]);
        else if ("prop" in col) col_getter.push(["prop", col.prop]);
      ...
      for (let item of col_getter) {
        ...
        if (col_type == "auto") {
          if (col_key === "name") {
            ...
            else if ("name" in this.entity) raw_content.push(this.entity.name);
            else if ("name" in this.entity.attributes)
              raw_content.push(this.entity.attributes.name);
            else raw_content.push(this.entity.entity_id);
          } else if (col_key === "object_id") {
            // return Object ID ('entity_id' after 1st dot)
            raw_content.push(
              this.entity.entity_id.split(".").slice(1).join(".")
            );
          } else if (
            col_key === "_state" &&
            "state" in this.entity.attributes
          ) {
          ...
          raw_content.push(
            col_key in this.entity.attributes
              ? this.entity.attributes[col_key]
              : null
          );
          ...
          if (col_key == "object_id") {
            ...
            raw_content.push(
              col_key in this.entity ? this.entity[col_key] : null
            );
      ...
      // new approach, KEEP AFTER DEPRECATION: (maybe without 'else' working anyways?!)
        ...
        raw_content = raw_content.map((obj) => String(obj)).join(delim);
      else raw_content = raw_content[0];

      let fmt = new CellFormatters(this.hass);
      if (col.fmt) {
        raw_content = fmt[col.fmt](raw_content);
        if (fmt.failed) raw_content = null;
      }
      return [null, undefined].every((x) => raw_content !== x)
        ? raw_content
        : new Array();
    });
    return null;
...
class FlexTableCard extends HTMLElement {
  ...
  _getRegEx(pats, invert = false) {
    ...
    if (invert) return new RegExp(`^(?:(?!${merged}).)*$`, "gi");
    else return new RegExp(`^${merged}$`, "gi");
  }

  _getEntities(hass, entities, incl, excl) {
    const format_entities = (e) => {
      if (!e) return null;
      if (typeof e === "string") return { entity: e.trim() };
      return e;
    };
  ...
  setConfig(config) {
    ...
    if ("css" in cfg) {
      for (var key in cfg.css) {
        ...
        else css_styles[css_key] = cfg.css[key];
      }
    }
    ...
    let my_headers = this.tbl.headers.map(
      (obj, idx) =>
        new Object({
          th_html_begin: `<th class="${cfg.columns[idx].align || "left"}">`,
          th_html_end: `${obj.name}</th>`,
          icon_html: obj.icon
            ? `<ha-icon id='icon' icon='${obj.icon}'></ha-icon>`
            : "",
        })
    );
                    ...
                    <thead>
                        <tr>
                            ${my_headers
                              .map(
                                (obj, idx) =>
                                  `${obj.th_html_begin}${obj.icon_html}${obj.th_html_end}`
                              )
                              .join("")}
                        </tr>
                    </thead>
  ...
  _updateContent(element, rows) {
    ...
    element.innerHTML = rows
      .map(
        (row) =>
          `<tr id="entity_row_${row.entity.entity_id}">${row.data
            .map((cell) =>
              !cell.hide
    ...
    rows.forEach((row) => {
      const elem = this.shadowRoot.getElementById(
        `entity_row_${row.entity.entity_id}`
      );
  ...
  set hass(hass) {
    ...
    let entities = this._getEntities(
      hass,
      config.entities,
      config.entities.include,
    ...
    let raw_rows = entities.map(
      (e) => new DataRow(e, config.strict, null, hass)
    );
      ...
      if (!row_obj.has_multiple) this.tbl.add(row_obj);
        ...
        this.tbl.add(
          ...transpose(row_obj.raw_data).map(
            (new_raw_data) =>
              new DataRow(row_obj.entity, row_obj.strict, new_raw_data, hass)
          )
        );
    });
