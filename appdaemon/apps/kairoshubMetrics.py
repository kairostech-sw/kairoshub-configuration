...
class KairoshubMetrics(hass.Hass):
    ...
    def systemMetrics(self, event_name, data, kwargs):
        ...
        statMessage["disk_use"] = self.get_state(
            "sensor.stat_disk_use", default=-1.0)
        statMessage["memory_use"] = self.get_state(
            "sensor.stat_memory_use", default=-1.0)
        statMessage["processor_use"] = self.get_state(
            "sensor.stat_processor_use", default=-1.0)
        statMessage["processor_temperature"] = self.get_state(
            "sensor.stat_processor_temperature", default=-1.0)
        statMessage["last_boot"] = self.get_state(
            "sensor.last_boot", default="")
        statMessage["processor_max_use"] = self.get_state(
            "sensor.stat_processor_use", attribute="max_value", default=-1.0)
    ...
    def entityMetrics(self, event_name, data, kwargs):
        ...
        if thermostat != "unknown":
            entityMessage["thermostat"] = thermostat
        if roller != "unknown":
        ...
        entityMessage["atHome"] = int(
            self.get_state("input_boolean.at_home") == "on")
        ...
        for group in self.get_state("group.zones", attribute="entity_id"):
            ...
            for room in zone:
                ...
                if entity_state != "unknown":
                    ...
                    thermostats_state["last_update"] = self.get_state(
                        "sensor.tz"+room_id, attribute="last_updated")
                ...
                if entity_state != "unknown":
                    ...
                    rollers_state["last_update"] = self.get_state(
                        "sensor.rz"+room_id, attribute="last_updated")
                ...
                if entity_state != "unknown":
                    ...
                    humidity_state["last_update"] = self.get_state(
                        "sensor.hz"+room_id, attribute="last_updated")
                    humidity.append(humidity_state)

                entity_state = self.get_state(
                    "binary_sensor.lz"+room_id+"_state")
        ...
        entityMessage["heating"]["state"] = int(
            self.get_state("switch.sw_thermostat") == "on")

        active_program = heatingManager.HeatingManager.isProgramOn(self, 0)

        if active_program > 0:
            entityMessage["heating"]["program"] = "program {}".format(
                active_program)
            entityMessage["heating"]["target"] = self.get_state(
                "input_number.temperature_period{}".format(active_program))
        else:
