...
class HelloWorld(hass.Hass):
    ...
    def ha_event(self, event_name, data, kwargs):
        ...
        if not currentSystemCode.startswith("H") and (mqttSystemCode == "" or mqttSystemCode == "unknown" or mqttSystemCode == "undefined"):
            self.log(
                "ERROR, system code not valid!! falling back into mainteneance mode", level="ERROR")
        ...
        if "unknown" == mqttSystemCode and currentSystemCode != "" and currentSystemCode.startswith("H"):
            ...
            self.fire_event(
                "AD_MQTT_PUBLISH", topic="kairostech/system_code", payload=currentSystemCode)
        ...
        if "unknown" == mqttSystemOwner and currentSystemOwner != "":
            ...
            self.fire_event(
                "AD_MQTT_PUBLISH", topic="kairostech/owner", payload=currentSystemOwner)
        ...
        khConfSwLastCheck = self.get_state(
            KAIROSHUB_CONFIGURATION_SW_LAST_CHECK)
        ...
        self.log("Retrieved global state for the application. state: %s",
                 hassState, level="INFO")

        hassCopy = hassState.copy()
        state = hassCopy.pop("state")
        hassCopy["version"] = hassCopy["kairoshub_version"] + \
            "#" + hassCopy["kairoshub_configuration_version"]
        self.set_state("sensor.kairos_home_state",
                       state=state, attributes=hassCopy)
        ...
        self.fire_event("AD_MQTT_PUBLISH", topic="kairostech/command",
                        payload="CHECK_INTERNET_CONNECTION")
        ...
        self.fire_event("HAKAFKA_PRODUCER_PRODUCE",
                        topic="TECHNICAL", message=deviceRequest)
