...
class KairoshubRollers(hass.Hass):
    ...
    def setRollersPosition(self, event_name: str, data: dict, kwargs: dict) -> None:
        ...
        positionNightzone = int(
            float(self.get_state(f"input_number.position_rs100_{mode}")))
        positionDayzone = int(float(self.get_state(
            f"input_number.position_rs200_{mode}")))

        for zone in zones:
            if self.getEntityId(zone)[0] == "1":
                self.setPosition(zone, positionNightzone,
        ...
        notyInfo = {
            ...
            "ncode": "ROLLES_CLOSED" if mode == "notathome" else "ROLLES_OPENED",
            "sender": sender,
            "severity": "NOTICE",
            "kwargs": {
                "mode": mode,
    ...
    def rollersProgram(self, event_name: str, data: dict, kwargs: dict) -> None:
        ...
        now = datetime.strptime(self.get_state(
            "sensor.date_time_iso"), self.datetimeFormat)
    ...
    def setProgramPosition(self, mode: str, sender: str, rollers: list, trid="") -> None:
        ...
        for roller in rollers:
            if self.getEntityId(roller)[0] == "1":
                self.setPosition(roller, positionNightzone,
                                 sendNotification=False)
            if self.getEntityId(roller)[0] == "2":
                self.setPosition(roller, positionDayzone,
                                 sendNotification=False)
    ...
    def setPosition(self, target: str, position: int, notyInfo={}, sendNotification=True) -> None:
        ...
        self.call_service("cover/set_cover_position",
                          entity_id=target, position=position)
        if sendNotification:
            self.fire_event("AD_KAIROSHUB_NOTIFICATION", **notyInfo)
